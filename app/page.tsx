...
import { Copy, TrendingUp, Zap, RefreshCw, AlertCircle } from "lucide-react"
...
export default function VexMemecoin() {
  ...
  const copyContract = () => {
    const contractAddress = TOKEN_CONFIG.address || "VEX7efb74a8d5ba2ddc580a0f07121a0e"
  ...
  return (
    <div className="min-h-screen bg-[#0a0f07] text-[#a8d5ba]">
      ...
      <section className="py-12 px-4 text-center relative overflow-hidden">
        ...
        <div className="container mx-auto relative z-10 max-w-4xl">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center mb-8">
            <Button className="bg-gradient-to-r from-[#7efb74] to-[#2ddc58] hover:from-[#2ddc58] hover:to-[#7efb74] text-black font-bold pixel-font px-6 py-3 shadow-[0_0_20px_#2ddc58]">
          ...
          {/* Contract Address */}
          <div className="bg-[#121a0e] p-3 rounded-lg border border-[#2ddc58]/50 max-w-md mx-auto shadow-[0_0_15px_#2ddc58]/30">
            <div className="flex items-center justify-center gap-2">
              <span className="text-xs pixel-font text-[#a8d5ba]">Contract:</span>
              <code className="text-[#7efb74] pixel-font text-xs">
                {TOKEN_CONFIG.address
                  ? `${TOKEN_CONFIG.address.slice(0, 6)}...${TOKEN_CONFIG.address.slice(-6)}`
                  : "VEX7efb...121a0e"}
              </code>
              <Button
                size="sm"
                ...
                className="text-[#2ddc58] hover:text-[#7efb74] p-1 h-auto"
              >
      ...
      {/* Stats Section */}
      <section className="py-10 px-4 bg-[#121a0e]/50">
        <div className="container mx-auto max-w-4xl">
          {TOKEN_CONFIG.enableLiveData && (
            <div className="text-center mb-4">
              <p className="text-xs text-[#a8d5ba] pixel-font">
                {tokenStats.lastUpdated
                  ? `Last updated: ${tokenStats.lastUpdated.toLocaleTimeString()}`
                  : tokenStats.error
                    ? `Note: ${tokenStats.error}`
                    : "Loading data..."}
                {tokenStats.tokenName && TOKEN_CONFIG.isExample && (
                  <span className="text-[#ff6b6b]">
                    {" "}
                    (Demo: {tokenStats.tokenName}
                    {tokenStats.tokenSymbol ? ` / $${tokenStats.tokenSymbol}` : ""})
                  </span>
                )}
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {statsData.map((stat, index) => (
              <Card
                key={index}
                className="bg-[#0f1a0d] text-[#a8d5ba] border-[#2ddc58]/50 shadow-[0_0_10px_#2ddc58]/20 hover:shadow-[0_0_20px_#2ddc58]/40 transition-all duration-300 relative"
              >
                <CardContent className="p-4 text-center">
                  {tokenStats.isLoading && (
                    <div className="absolute inset-0 bg-[#0f1a0d]/80 flex items-center justify-center">
                      <RefreshCw className="w-4 h-4 animate-spin text-[#2ddc58]" />
                    </div>
                  )}
                  {typeof stat.icon === "string" ? (
                    <div className="text-xl mb-1">{stat.icon}</div>
                  ) : (
                    <stat.icon className="w-5 h-5 mx-auto mb-1 text-[#7efb74]" />
                  )}
                  <div className={`text-lg font-bold mb-1 pixel-font ${stat.color}`}>{stat.value}</div>
                  <div className="text-xs text-[#a8d5ba] pixel-font">{stat.label}</div>
                </CardContent>
              </Card>
            ))}
          </div>
